...
contains
    ...
    function write_lut(filename, uLUT, vLUT, dz, options) result(error)
        ...
        if (file_exists(filename)) then
            if (STD_OUT_PE) write(*,*) "WARNING: Linear Theory look-up-table file already exists."
            error = 1
            if (.not.options%overwrite_lt_lut) then
                if (STD_OUT_PE) write(*,*) "WARNING: Existing file will not be overwritten."
                return
            endif
            if (STD_OUT_PE) write(*,*) "WARNING: Existing file *will* be overwritten."
        endif
        ...
        if (error/=0) then
            if (STD_OUT_PE) write(*,*) "Error writing uLUT to file:"//trim(filename)//" Error code = "//trim(str(error))
            return
        endif
        error = write_var(filename,"vLUT",vLUT, dimnames=[character(len=4) :: "nspd","ndir","nnsq","nx","nz","nyv"])
        if (error/=0) then
            if (STD_OUT_PE) write(*,*) "Error writing vLUT to file:"//trim(filename)//" Error code = "//trim(str(error))
            return
        endif

        error = write_var(filename,"dz",dz, dimnames=[character(len=4) :: "nz"])
        if (error/=0) then
            if (STD_OUT_PE) write(*,*) "Error writing dz to LUT file:"//trim(filename)//" Error code = "//trim(str(error))
            return
        endif
    ...
    function setup_dim(ncid, dimname, dimid, n) result(error)
        ...
        if (ncerror/=NF90_NOERR) then
            ...
            if (dim_length/=n) then
                error = 1
                if (STD_OUT_PE) write(*,*) "Dim setup error: ", trim(dimname), dimid, dim_length,"!=",n
            endif
        endif
        ! return error
    ...
    function check_attribute_r(filename, default_value, value_name) result(error)
        ...
        if (error/=NF90_NOERR) then
            error=1
            if (STD_OUT_PE) write(*,*) "WARNING: Error reading attribute: ",trim(value_name)," from: ",trim(filename)
        else
            if (default_value/=test_value) then
                error = 1
                if (STD_OUT_PE) write(*,*) "WARNING parameter option: "//trim(value_name)//"  "//trim(str(default_value)), &
    ...
    function check_attribute_c(filename, default_value, value_name) result(error)
        ...
        if (error/=NF90_NOERR) then
            ...
            if (default_value/=test_value) then
                error = 1
                if (STD_OUT_PE) write(*,*) "WARNING parameter option: "//trim(value_name)//"  "//trim(default_value), &
