    ...
    subroutine update_winds(domain, forcing, options)
            ...
            if (options%physics%windtype==kLINEAR_ITERATIVE_WINDS .or. options%physics%windtype==kITERATIVE_WINDS) then
                ...
                if (options%wind%alpha_const>0) then
                    do i = ims,ime
                        do k = kms, kme
                            do j = jms, jme
                                domain%vars_3d(domain%var_indx(kVARS%wind_alpha)%v)%data_3d(i,k,j) = options%wind%alpha_const
                            enddo
                        enddo
                    enddo
                else
        ...
        if (.not.(options%forcing%wvar=="")) then
            ...
            do i = ims,ime
                do k = kms, kme
                    do j = jms, jme
                        domain%vars_3d(domain%var_indx(kVARS%w_real)%v)%dqdt_3d(i,k,j) = 0.0
                    enddo
                enddo
            enddo
        endif
